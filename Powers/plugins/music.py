...
async def mhelp(_, m: Message):
    await m.reply_photo(
        photo=str(choice(StartPic)),
        caption=f"{__HELP__}",
        quote=True,
        reply_markup=(await gen_music_kb(m)),
    )
...
async def send_music_help(_, q: CallbackQuery):
    await q.edit_message_caption(
        caption="""Music
...
Gojo supports a large number of formatting options to make your messages more expressive. Take a look by clicking the buttons below!""",
        reply_markup=(await gen_music_kb(q.message)),
    )
    await q.answer()
    return
...
__alt_name__ = ["music", "mhelp", "madmin"]
__buttons__ = [
    [
        ("Admin", "md_music"),
        ("Play", "madmin"),
